...
# Titre :    SheetCreatorX - Version Linux
...
# Prérequis : Google Chromium, LaTeX Distribution (pour PDFLaTeX), Editeur LaTeX
...
def Generer():
 ...
 car == "ø" or car == "%" or car == "µ" or car == "*":
	...
	if ModeAvanceeBool is True:
		...
		Commande = "ls | grep \'"+NomListeMots+"\'"
		ListeTrouveeBrut = os.system(Commande)
		ListeTrouvee = str(ListeTrouveeBrut)
		ListeTrouveeListe = ListeTrouvee.split("\n")
		if len(ListeTrouveeListe) == 0:
	...
	CommandeCreaDossier = "mkdir Fichiers\ Créés/"+MotRef
	os.system(CommandeCreaDossier)
	# On change le CWD pour le dossier venant d'être créé
	CommandeChangerDossier = "./Fichiers Créés/"+MotRef
	os.chdir(CommandeChangerDossier)
	...
	ListeTProche = open(NomListeTProche, "w", encoding="utf-8")
	ListeProche = open(NomListeProche, "w", encoding="utf-8")
...
== False:
	...
	ListeTProche = open(NomListeTProche, "r", encoding="utf-8")
	ListeProche = open(NomListeProche, "r", encoding="utf-8")
...
+","+str((len(ContenuListeProche)))+","+str((len(ContenuListeRef))))
	...
	FichierTex = open(NomFichierTex, "w", encoding="utf-8")
...
\n")
	...
	Commande3 = "chromium-browser \"`pwd`/"+NomFichierTex[:-3]+"pdf\""
...
def Aide():
	try:
		os.system("chromium-browser \"/`pwd`/Aide.pdf\"")
	except:
